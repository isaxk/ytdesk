...
class Store<TSchema> {
  ...
  public async get(_key: keyof TSchema) {
  ...
  public onDidAnyChange(
    callback: (newState: TSchema, oldState: TSchema) => void,
  ) {
    return ipcRenderer.on(
      "settings:stateChanged",
      (_event, newState, oldState) => {
...
contextBridge.exposeInMainWorld("ytmd", {
  sendVideoProgress: (volume: number) =>
    ipcRenderer.send("ytmView:videoProgressChanged", volume),
  sendVideoState: (state: number) =>
    ipcRenderer.send("ytmView:videoStateChanged", state),
  sendVideoData: (
...
window.addEventListener("load", async () => {
  ...
  const integrationScripts: {
    [integrationName: string]: { [scriptName: string]: string };
  } = null;
  ...
  ipcRenderer.on("ytmView:getPlaylists", async (_event, requestId) => {
    const rawPlaylists = await (
      await webFrame.executeJavaScript(getPlaylistsScript)
    )();
  ...
  ipcRenderer.on(
    "ytmView:executeScript",
    async (_event, integrationName, scriptName) => {
...
ipcRenderer.send("ytmView:videoDataChanged", null);
